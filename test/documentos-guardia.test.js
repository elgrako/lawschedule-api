 ...
 * guardia_id — misma logica de subida/descarga/borrado, reusando el helper
 * compartido src/lib/uploads.js. El contenido se guarda como BYTEA en la
 * propia fila (Postgres/Neon), no en disco. La proteccion IDOR de
 * :guardiaId (router.param -> ownsGuardia) se prueba aparte en
...
await testAsync('POST /:guardiaId/documentos con fichero valido -> 201, inserta el contenido como BYTEA', async () => {
    const handler = findHandler(guardiasRouter, 'post', '/:guardiaId/documentos');
    let updateCalls = 0;
...
await testAsync('POST /:guardiaId/documentos con error de BD -> 500', async () => {
    const handler = findHandler(guardiasRouter, 'post', '/:guardiaId/documentos');
...
await testAsync('GET /:guardiaId/documentos/:docId/file cuya fila no tiene contenido guardado -> 404', async () => {
    const handler = findHandler(guardiasRouter, 'get', '/:guardiaId/documentos/:docId/file');
...
await testAsync('GET /:guardiaId/documentos/:docId/file existente -> sirve el contenido con headers de seguridad', async () => {
    const handler = findHandler(guardiasRouter, 'get', '/:guardiaId/documentos/:docId/file');
    const fileBuffer = Buffer.from('contenido-fake-pdf');
    ...
    const res = fakeRes();
    await handler(req, res);
    assert.strictEqual(res._sentBody, fileBuffer);
